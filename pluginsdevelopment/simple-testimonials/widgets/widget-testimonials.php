...
class Testimonial_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$owlc_opts = ( null == $instance['owlc_opts'] ) ? '' : $instance['owlc_opts'];
		echo $before_widget;

		if ( ! empty( $title ) )
			echo $before_title . $title . $after_title;

		echo get_testimonial( $posts_per_page, $orderby, $testimonial_id, $owlc_opts);
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['owlc_opts'] = ( null == $new_instance['owlc_opts'] ) ? '' : $new_instance['owlc_opts'];
		return $instance;
	}

	public function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'posts_per_page' => '1', 'orderby' => 'none', 'testimonial_id' => null, 'owlc_opts' => null) );
		...
		<p><label for="<?php echo $this->get_field_id( 'owlc_opts' ); ?>">Owlc Options:</label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'owlc_opts' ); ?>" name="<?php echo $this->get_field_name( 'owlc_opts' ); ?>" type="text" value="<?php echo esc_attr_e($owlc_opts); ?>" /></p>
